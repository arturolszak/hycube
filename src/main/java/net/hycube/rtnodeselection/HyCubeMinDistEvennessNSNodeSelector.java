...
public class HyCubeMinDistEvennessNSNodeSelector extends HyCubeNSNodeSelector {
	...
	public void processNode(NodePointer newNode,
		...
		if (ns.size() < nsSize) {
			...
			double minDist = 0;
			double[] minDists = new double[ns.size()];
			for (int i = 0; i < ns.size() - 1; i++) {
				for (int j = i + 1; j < ns.size(); j++) {
					...
					if (minDist == 0 || distij < minDist) minDist = distij;
					for (int k = 0; k < ns.size(); k++) {
						if (i == k) {
							...
							if (minDists[k] == 0 || distkj < minDists[k]) minDists[k] = distkj;
						}
						if (j == k) {
							...
							if (minDists[k] == 0 || distik < minDists[k]) minDists[k] = distik;
						}
						else {
							if (minDists[k] == 0 || distij < minDists[k]) minDists[k] = distij;
						}
					}
				}
			}
			
			currEvenness = minDist;
			newEvenness = minDists;
			
			if (nsNodeSelectionAlgorithmEvennessCompElemExp != 1) {
				currEvenness = Math.pow(currEvenness, nsNodeSelectionAlgorithmEvennessCompElemExp);
				for (int i = 0; i < ns.size(); i++) newEvenness[i] = Math.pow(newEvenness[i], nsNodeSelectionAlgorithmEvennessCompElemExp);
			}
