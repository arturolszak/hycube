...
public class HyCubeReceivedMessageProcessorNotify implements ReceivedMessageProcessor {
	...
	private static org.apache.commons.logging.Log devLog = LogHelper.getDevLog(HyCubeReceivedMessageProcessorNotify.class); 
	
	
	protected static final String PROP_KEY_MESSAGE_TYPES = "MessageTypes";
	protected static final String PROP_KEY_VALIDATE_NOTIFY_MESSAGE_SENDER = "ValidateNotifyMessageSender";
	
	
	
	protected boolean validateNotifyMessageSender;
	...
	public void initialize(NodeAccessor nodeAccessor, NodeProperties properties) throws InitializationException {
		...
		if (devLog.isDebugEnabled()) {
			devLog.debug("Initializing HyCubeReceivedMessageProcessorNotify.");
		}
		...
		try {
			...
			validateNotifyMessageSender = (Boolean) properties.getProperty(PROP_KEY_VALIDATE_NOTIFY_MESSAGE_SENDER, MappedType.BOOLEAN);
	...
	public boolean processMessage(Message message, NetworkNodePointer directSender) throws ProcessMessageException {
		...
		try {
			switch (msg.getType()) {
				case NOTIFY:
					processNotifyMessage(msg, directSender);
					break;
				default:
					break;
			}
	...
	protected void processNotifyMessage(Message msg, NetworkNodePointer directSender) throws NetworkAdapterException, ProcessMessageException {
    	
    	if (msg.getRecipientId().equals(nodeAccessor.getNodeId())) {
			if (devLog.isDebugEnabled()) {
				devLog.debug("Received NOTIFY message #" + msg.getSerialNoAndSenderString() + ".");
			}
			if (msgLog.isInfoEnabled()) {
				msgLog.info("Received NOTIFY message #" + msg.getSerialNoAndSenderString() + ".");
			}
			
		
			//process the notify message
			if (devLog.isDebugEnabled()) {
				devLog.debug("Processing NOTIFY message #" + msg.getSerialNoAndSenderString() + " from node " + msg.getSenderId().toHexString() + ".");
			}
			if (msgLog.isInfoEnabled()) {
				msgLog.info("Processing NOTIFY message #" + msg.getSerialNoAndSenderString() + " from node " + msg.getSenderId().toHexString() + ".");
			}
			
			
			if (validateNotifyMessageSender) {
				...
				if (! java.util.Arrays.equals(directSender.getAddressBytes(), (msg.getSenderNetworkAddress()))) {
					//drop the message:
					if (devLog.isDebugEnabled()) {
						devLog.debug("NOTIFY message #" + msg.getSerialNoAndSenderString() + " dropped. Received from another node than the LEAVE sender.");
					}
					if (msgLog.isInfoEnabled()) {
						msgLog.info("NOTIFY message #" + msg.getSerialNoAndSenderString() + " dropped. Received from another node than the LEAVE sender.");
					}
					return;
				}
			}
			...
			NodeId newNodeId = msg.getSenderId();
			byte[] newNodeNetworkAddress = msg.getSenderNetworkAddress();
			
	    	NodePointer newNodePointer = new NodePointer(nodeAccessor.getNetworkAdapter(), newNodeNetworkAddress, newNodeId);
	    	long currTimestamp = nodeAccessor.getEnvironment().getTimeProvider().getCurrentTime();
	    	nodeAccessor.getNotifyProcessor().processNotify(newNodePointer, currTimestamp);
