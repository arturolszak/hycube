...
public class HyCubeReceivedMessageProcessorSearchJoin implements ReceivedMessageProcessor {
	...
	private static org.apache.commons.logging.Log devLog = LogHelper.getDevLog(HyCubeReceivedMessageProcessorSearchJoin.class); 
	...
	protected HyCubeSearchJoinManager joinManager;
	...
	public void initialize(NodeAccessor nodeAccessor, NodeProperties properties) throws InitializationException {
		...
		try {
			...
			if (nodeAccessor.getJoinManager() instanceof HyCubeSearchJoinManager) {
				this.joinManager = (HyCubeSearchJoinManager) nodeAccessor.getJoinManager();
			}
			else {
				throw new InitializationException(InitializationException.Error.NODE_INITIALIZATION_ERROR, null, "Unable to initialize received message processor instance. The join manager is expected to be an instance of: " + HyCubeSearchJoinManager.class.getName() + ".");
			}
	...
	protected void processJoinMessage(Message msg, NetworkNodePointer directSender) {
		if (devLog.isDebugEnabled()) {
		...
		HyCubeSearchJoinMessageData joinData = null;
		try {
			joinData = HyCubeSearchJoinMessageData.fromBytes(msg.getData(), nodeIdDimensions, nodeIdDigitsCount);
		...
		this.joinManager.processJoinRequest(sender, joinData.getJoinId(), joinData.getJoinNodeId(), joinData.getParameters(), directSender, joinData.getDiscoverPublicNetworkAddress());
		
	}
	
	
	protected void processJoinReplyMessage(Message msg) {
		if (devLog.isDebugEnabled()) {
		...
		HyCubeSearchJoinReplyMessageData joinReplyData = null;
		try {
			joinReplyData = HyCubeSearchJoinReplyMessageData.fromBytes(msg.getData(), nodeIdDimensions, nodeIdDigitsCount, networkAddressByteLength);
		} catch (MessageByteConversionException e) {
			if (devLog.isDebugEnabled()) {
		...
		this.joinManager.processJoinResponse(sender, joinReplyData.getJoinId(), requestorNetworkNodePointer, joinReplyData.getParameters(), nodesFound);
