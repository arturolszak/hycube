...
public class HyCubeLeaveMessageData {
		
	
	protected int calculateLeaveMessageDataLength(int nodeIdDimensions, int nodeIdDigitsCount) {
		int dataLength =
	...
	protected HyCubeLeaveMessageData() {
		
	}
	
	public HyCubeLeaveMessageData(byte[][] networkAddresses, HyCubeNodeId[] nodeIds) {
	...
	public HyCubeLeaveMessageData(NodePointer[] nodePointers) {
	...
	public byte[] getBytes(int nodeIdDimensions, int nodeIdDigitsCount) {
		...
		ByteBuffer b = ByteBuffer.allocate(calculateLeaveMessageDataLength(nodeIdDimensions, nodeIdDigitsCount));
	...
	public static HyCubeLeaveMessageData fromBytes(byte[] bytes, int nodeIdDimensions, int nodeIdDigitsCount, int networkAddressByteLength) throws MessageByteConversionException {
		
		HyCubeLeaveMessageData leaveMsg = new HyCubeLeaveMessageData();
		...
		try {
			...
			leaveMsg.networkAddresses = networkAddresses;
			leaveMsg.nodeIds = nodeIds;
		...
		if (leaveMsg.calculateLeaveMessageDataLength(nodeIdDimensions, nodeIdDigitsCount) != bytes.length) {
			throw new MessageByteConversionException("The length of the byte array passed to the method is not equal to the expected message data length.");
		}
